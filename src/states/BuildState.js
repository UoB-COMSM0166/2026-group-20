...
export class BuildState extends State {
    ...
    static PALETTE = [
        ...
        // Sprite hazard
        { type: ObstacleType.SPIKED_BALL,     label: 'SpikedBall',  hint: 'Kills on touch',   color: [180, 60,  60]  },
    ];
    ...
    _drawGhost(p, type, x, y) {
        ...
        switch (type) {
            case ObstacleType.PLATFORM:         Platform.drawGhost(p, x, y, s.platform);                       break;
            case ObstacleType.MOVING_PLATFORM:  MovingPlatform.drawGhost(p, x, y, s.movingPlatform);           break;
            case ObstacleType.FALLING_PLATFORM: FallingPlatform.drawGhost(p, x, y, s.fallingPlatform);         break;
            case ObstacleType.ICE_PLATFORM:     IcePlatform.drawGhost(p, x, y, s.icePlatform);                 break;
            case ObstacleType.BOUNCE_PAD:       BouncePad.drawGhost(p, x, y, s.trampoline);                    break;
            case ObstacleType.SPIKE:            SpikeObstacle.drawGhost(p, x, y);                              break;
            case ObstacleType.CANNON:           Cannon.drawGhost(p, x, y, this._cannonDir, s.cannon);          break;
            case ObstacleType.SAW:              Saw.drawGhost(p, x, y, s.saw);                                 break;
            case ObstacleType.FLAME:            Flame.drawGhost(p, x, y, s.fire);                              break;
            case ObstacleType.SPIKE_PLATFORM:   SpikePlatform.drawGhost(p, x, y, s.spikePlatform);             break;
            case ObstacleType.ICE_BLOCK:        IceBlock.drawGhost(p, x, y, s.iceBlock);                       break;
            case ObstacleType.WIND_ZONE:        WindZone.drawGhost(p, x, y, this._windDir, s.windZone);        break;
            case ObstacleType.TELEPORTER:       Teleporter.drawGhost(p, x, y, s.teleporter);                   break;
            case ObstacleType.SPIKED_BALL:      SpikedBall.drawGhost(p, x, y, s.spikedBall);                   break;
        }
    }
    ...
    _createObstacle(type, x, y) {
        ...
        const s = this.ctx.obstacleSprites;
        switch (type) {
            case ObstacleType.PLATFORM:         return new Platform(p, x, y, s.platform);
            case ObstacleType.MOVING_PLATFORM:  return new MovingPlatform(p, x, y, s.movingPlatform);
            case ObstacleType.FALLING_PLATFORM: return new FallingPlatform(p, x, y, s.fallingPlatform);
            case ObstacleType.ICE_PLATFORM:     return new IcePlatform(p, x, y, s.icePlatform);
            case ObstacleType.BOUNCE_PAD:       return new BouncePad(p, x, y, s.trampoline);
            case ObstacleType.SPIKE:            return new SpikeObstacle(p, x, y);
            case ObstacleType.CANNON:           return new Cannon(p, x, y, this._cannonDir, s.cannon);
            case ObstacleType.SAW:              return new Saw(p, x, y, s.saw);
            case ObstacleType.FLAME:            return new Flame(p, x, y, s.fire);
            case ObstacleType.SPIKE_PLATFORM:   return new SpikePlatform(p, x, y, s.spikePlatform);
            case ObstacleType.ICE_BLOCK:        return new IceBlock(p, x, y, s.iceBlock);
            case ObstacleType.WIND_ZONE:        return new WindZone(p, x, y, this._windDir, s.windZone);
            case ObstacleType.TELEPORTER:       return new Teleporter(p, x, y, s.teleporter);
            case ObstacleType.SPIKED_BALL:      return new SpikedBall(p, x, y, s.spikedBall);
            default:                            return null;
        }
    }
}
